    ...
    public static class QueryManager
    {
        ...
        public static void QueryDevices(IMessageNotifier messageNotifier)
        {
            ...
            else
            {
                ...
                AmdDevices = amd.QueryAmd(_isOpenCLQuerySuccess, _openCLJsonData, messageNotifier);
            }
            // #5 uncheck CPU if GPUs present, call it after we Query all devices
            Available.UncheckedCpu();
            ...
            double totalSysRam = ComputeDeviceManager.SystemSpecs.FreePhysicalMemory + ComputeDeviceManager.SystemSpecs.FreeVirtualMemory;
            // check
        ...
        private static class Cpu
        {
            public static void QueryCpus()
            {
                ...
                if (CpuUtils.IsCpuMiningCapable())
                {
                    ...
                    {
                        Available.Add(
                            new Device.CpuComputeDevice(0, "CPU0", CpuID.GetCpuName().Trim(), threadsPerCpu, 0,
                                ++CpuCount)
                        );
                    }
                    else if (Available.CpusCount > 1)
                    {
                        ...
                        {
                            Available.Add(
                                new Device.CpuComputeDevice(i, "CPU" + i, CpuID.GetCpuName().Trim(), threadsPerCpu,
                ...
                if (_cudaDevices != null && _cudaDevices.Count != 0)
                {
                    ...
                    foreach (var cudaDev in _cudaDevices)
                    {
                        ...
                        if (!skip)
                        {
                            ...
                            Available.Add(
                                new Device.CudaComputeDevice(cudaDev, group, ++GpuCount, handle, nvmlHandle)
                            );
                        }
                    }
                    Helpers.ConsolePrint(Tag, stringBuilder.ToString());
