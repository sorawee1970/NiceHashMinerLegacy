...
using NiceHashMinerLegacy.Windows.Device;
...
namespace NiceHashMinerLegacy.Windows.Querying
{
    public class AmdQuery
    {
        ...
        public List<OpenCLDevice> QueryAmd(bool openCLSuccess, IEnumerable<OpenCLJsonData> openCLData, IMessageNotifier note)
        {
        ...
        private void DriverCheck(IMessageNotifier note)
        {
        ...
        private List<OpenCLDevice> ProcessDevices(IEnumerable<OpenCLJsonData> openCLData)
        {
            ...
            foreach (var oclEl in openCLData)
            {
                ...
                Helpers.ConsolePrint(Tag,
                    $"AMD platform found: Key: {amdOpenCLPlatformStringKey}, Num: {Available.AmdOpenCLPlatformNum}");
                break;
            }
        ...
        private List<OpenCLDevice> AmdDeviceCreationPrimary(List<OpenCLDevice> amdDevices)
        {
            ...
            foreach (var dev in amdDevices)
            {
                ...
                if (busID != -1 && _busIdInfos.ContainsKey(busID))
                {
                    ...
                    Available.Add(
                        new AmdComputeDevice(newAmdDev, ++QueryManager.GpuCount, false,
        ...
        private List<OpenCLDevice> AmdDeviceCreationFallback(List<OpenCLDevice> amdDevices)
        {
            ...
            for (var i = 0; i < minCount; ++i)
            {
                ...
                Available.Add(
                    new AmdComputeDevice(newAmdDev, ++QueryManager.GpuCount, true, -1));
                // just in case 
                try
                {
                    stringBuilder.AppendLine($"\t{skipOrAdd} device{isDisabledGroupStr}:");
